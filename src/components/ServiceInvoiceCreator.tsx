...
export function ServiceInvoiceCreator({
    ...
    try {
      ...
      const payments = paid > 0
        ? [{
            ...
            notes: "Payment received before this invoice was created (for example, Zelle or another external payment).",
  ...
  const createStripeLink = async () => {
    ...
    const paymentAmount = amount
      ? servicePhaseAmounts(serviceDraft, fee)[serviceDraft.phases.findIndex((phase) => phase.name === amount.name)] ?? 0
      : 0;
                    ...
                    {serviceDraft.phases.map((phase, index) => {
                      ...
                      return (
                        <tr key={phase.name} className="border-b border-border">
                          ...
                          <td className="py-3 px-4 text-right min-w-[130px]">{formatMoney(servicePhaseAmounts(serviceDraft, fee)[index] ?? 0)}</td>
...
function servicePhaseAmounts(draft: ServiceInvoiceDraft, fee: number) {
  const remainingFee = roundMoney(Math.max(fee - paidDesignFee(draft, fee), 0));

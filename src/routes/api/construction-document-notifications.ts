...
const RECIPIENT_EMAILS = new Set([
...
async function authorize(request: Request) {
  ...
  if (!canUseEaWorkspace(profile) || !RECIPIENT_EMAILS.has(email)) {
    return {
      response: json(
        { error: "Construction document notices are available only to Ken, Katie, and Brynn." },
        403,
      ),
    };
  }
...
export const Route = createFileRoute("/api/construction-document-notifications")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const { data, error } = await admin
          ...
          .select(
            "notification_id,notification:studio_construction_document_notifications(id,project_document_id,project_id,project_name,file_name,created_at)",
          )
